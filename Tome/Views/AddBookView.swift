    ...
    private var contentView: some View {
        if viewModel.isLoading {
            ...
            if viewModel.searchQuery.isEmpty {
                ...
                // Still typing, haven't searched yet - show empty state
                VStack {
                    emptyState
                        .padding(.top, 40)
                    Spacer()
...
struct AddBookManualView: View {
    ...
    private var coverImageSection: some View {
        VStack(spacing: 12) {
            ...
            .help("Click to add or change cover image")
#endif
            
#if os(iOS)
            // iOS: Hint text
            Text(coverImageData == nil ? "Tap the image to add a cover" : "Tap to change or remove")
                .font(.caption2)
                .foregroundStyle(.tertiary)
                .multilineTextAlignment(.center)
#else
            // macOS: Hint text
            Text("Click the image to add a cover")
                .font(.caption2)
                .foregroundStyle(.tertiary)
                .multilineTextAlignment(.center)
#endif
